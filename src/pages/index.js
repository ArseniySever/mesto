...
import { validationConfig, initialCards, popupPhoto, popupSubtitle, placesBox, editButton, addButton, login, job, titleInput, imageInput, formEditProfile, formAddNewCard, nameInput, jobInput} from '../components/constants.js';
...
const placeInfoNew = new PlaceInfo({ nameLink: popupSubtitle, sourceLink: popupPhoto });
...
const cardSection = new Section({
  renderer: (item) => {
    const placeElement = addCard(item, ".element-template"); 
...
function addCard(cardData, template) {
...
;
  const getPlaceInfoNew = placeInfoNew.getUserInfo();

  const createdCard = addCard(getPlaceInfoNew, ".element-template");
...
const popupMestoElement = new PopupWithForm({
  ...
  handleFormSubmit: (formData) => {
    placeInfoNew.setUserInfo({name: formData.name, link: formData.link });
    const newCard = createPlace();
